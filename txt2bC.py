...
def parse(filename, whiten):
  ...
  C=(C + C.T) / 2
  return L, m_X, sig_X, b, C

#if __name__ == "__main__":
if len(sys.argv)<2:
  print "Usage: python txt2bC.py <input_file>"
else:
  L, m_X, sig_X, b,C = parse(sys.argv[1], whiten=yahoo_common.whiten)
  if yahoo_common.whiten:
    filename="data/yahoo.set2.train.bC.npz"
  else:
    filename="data/yahoo.set2.train.bC.no_whiten.npz"
  np.savez(filename, b=b, C=C, X_std=sig_X, X_mean=m_X, L_whiten=L)
